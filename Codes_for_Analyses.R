...
Adiv.LSmeans <- Adiv.LSmeans %>% 
  ...
  dplyr::mutate(Fraction = gsub(Fraction, pattern="SK",replacement = "SK fraction")) %>% 
  dplyr::mutate(Fraction = gsub(Fraction, pattern="SS",replacement = "SS fraction")) %>% 
  dplyr::mutate(Fraction = gsub(Fraction, pattern="FL",replacement = "FL fraction")) %>%
  transform(Layer = factor(Layer, levels = c("ML", "Z1%")), 
            Fraction = factor(Fraction, levels = c("SK fraction", "SS fraction", "FL fraction")))
head(Adiv.LSmeans)
...
write.table(Env.data.RDA, "./data/Env.data.RDA.txt", quote = F, sep = "\t", row.names = T, col.names = NA
            , append = F)
...
RDA.model.first
#Constrained Proportion: variance of Y explained by 38.5(%)
#Unconstrained Proportion: unexplained variance in 61.5(%)
RsquareAdj(RDA.model.first)
#the adjusted R2 (corrected for the number of explanatory variables): 0.3850182
